...
Stop the Open CAS Linux software as described in the section [**Stopping Cache Instances**](/open-cas-linux/running/#stopping-cache-instances).
...
Yes. See the section [**Manual Configuration for Write-Back Mode**](/open-cas-linux/configuring/#manual-configuration-for-write-back-mode) for details.
...
The *Reads from Cache* counter from the CAS statistic output ( See [*Monitoring CAS*](/open-cas-linux/monitoring/) ) can increase when only doing writes from the host in several cases.
