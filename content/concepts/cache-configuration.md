...
This page describes various [**cache**](/concepts/cache/) configuration
...
[**dirty data**](/concepts/cache-operations/)).
...
opportunistically (depending on [**cleaning policy**](/concepts/cleaning/)).
...
[**invalid**](/concepts/cache-line/). In this mode only read requests are
...
[**eviction**](/concepts/cache-operations/) operation for workloads where
...
[**read hits**](/concepts/cache-operations/) will be handled by reading data
from **cache storage** until all the cache lines will be
[**cleaned**](/concepts/cache-operations/).

## Cache line size

The **cache line size** parameter determines the size of a data block
on which the **cache** operates ([**cache line**](/concepts/cache-line/)).
It's the minimum portion of data on the **backend storage**
([**core line**](/concepts/cache-line/)) that can be mapped into the **cache**.
...
<small>[[TODO](/contributing/authoring/)] Add missing parameters</small>
